    ...
    private void deplacerCasesRecursif(Case[] extremites, int rangee, int direction, int compteur) {
        ...
        if (extremites[rangee] != null) {
            ...
            if (voisin != null) {
                ...
                if (extremites[rangee].valeurFib(voisin)) { // verifier valeurFib
    ...
    * Attention : le tableau retourné peut contenir des null si les lignes/colonnes sont vides
    ...
    public boolean nouvelleCase() {
        if (this.grille.size() < TAILLE * TAILLE) {
            ...
            if ((this.grille.size() == 1) || (this.valeurMax == 1 && ajout.getValeur() == 2d
                    )) { // Mise à jour de la valeur maximale présente dans la grille si c'est la première case ajoutée ou si on ajoute un 4 et que l'ancien max était 2
